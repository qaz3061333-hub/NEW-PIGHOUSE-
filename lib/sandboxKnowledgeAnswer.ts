...
export type SandboxKnowledgeAnswerRequest = {
  message?: string;
...
export class SandboxKnowledgeAnswerError extends Error {
  status: number;

  constructor(message: string, status = 500) {
    super(message);
    this.name = "SandboxKnowledgeAnswerError";
    this.status = status;
    Object.setPrototypeOf(this, SandboxKnowledgeAnswerError.prototype);
  }
}
...
function normalizeHistory(history: SandboxKnowledgeAnswerRequest["history"] = []) {
...
function buildContextualQuery(queryMessage: string, history: SandboxKnowledgeAnswerRequest["history"] = []) {
...
export async function runSandboxKnowledgeAnswer({
  message,
  analysisResult,
  history,
}: SandboxKnowledgeAnswerRequest): Promise<SandboxKnowledgeAnswer> {
  const queryMessage = compactText(message || "");

  if (!queryMessage) {
    throw new SandboxKnowledgeAnswerError("message 不可為空", 400);
  }
  ...
  if (!apiKey) {
    throw new SandboxKnowledgeAnswerError("Gemini API Key 尚未設定", 500);
  }

  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL?.trim();
  const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY?.trim();
  if (!supabaseUrl || !supabaseAnonKey) {
    throw new SandboxKnowledgeAnswerError("Supabase 環境變數未設定", 500);
  }
  ...
  if (error) {
    throw new SandboxKnowledgeAnswerError(`查詢知識庫失敗：${error.message}`, 500);
  }
...
${context}`;
  ...
  if (!response.ok) {
    ...
    throw new SandboxKnowledgeAnswerError(`Gemini 呼叫失敗（model: ${model}）：${body || response.statusText}`, 502);
  }
  ...
  if (!answer) {
    throw new SandboxKnowledgeAnswerError(`Gemini 未回傳可解析內容（model: ${model}）`, 502);
  }
